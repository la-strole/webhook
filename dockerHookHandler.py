...
def image_manipulation(imagename, options, commands):
    ...
    stop_result = subprocess.run(stop_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    if not stop_result.stderr:
        logger.info(f"Container {imagename} stopped.")
        # Remove image
        # wait for 5 seconds
        time.sleep(5)
        remove_image_command = ['sudo', 'docker', 'image', 'rm', f'{imagename}']
        remove_image_result = subprocess.run(remove_image_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        if not remove_image_result.stderr:
            logger.info(f"Image {imagename} removed.")
        else:
            logger.error(f"Failed to remove image {imagename}: {remove_image_result.stderr}")
    else:
        logger.error(f"Failed to stop container: {remove_image_result.stderr}")

    # Pull new image from dockerhub and run container
    pull_image_command = ['sudo', 'docker', 'image', 'pull', imagename]
    pull_image_result = subprocess.run(pull_image_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    if not pull_image_result.stderr:
        ...
        run_container_result = subprocess.run(run_contaiter_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        if not run_container_result:
            logger.info(f'Successfully run conatiner from image {imagename}')
        else:
            logger.error(f'Failed to run container from image {imagename}: {run_container_result.stderr}')

        return 0

    else:
        logger.error(f'Failed pull image {imagename} from dockerhub: : {pull_image_result.stderr}')
        return -1
